...
// Start server only once (do NOT use app.listen separately)
server.listen(PORT, () => {
...
// ========== Optional: DB Health Middleware (use only on important routes) ==========
...
// ========== 👇 Serve Swagger UI ==========
app.use('/wowomart-Task-api-docs', swaggerUi.serve, swaggerUi.setup(swaggerSpec));
...
// ========== (Optional) Monitor Pool Status ==========
setInterval(async () => {
    try {
        await wowomartPool.execute('SELECT 1');
        console.log('🧪 Wowomart DB: ✅ Alive');
    } catch (e) {
        console.error('🧪 Wowomart DB: ❌ Down', e.message);
    }

    try {
        await taskManagementPool.execute('SELECT 1');
        console.log('🧪 TaskManagement DB: ✅ Alive');
    } catch (e) {
        console.error('🧪 TaskManagement DB: ❌ Down', e.message);
    }
}, 30000);
// Log every 30 seconds (for dev/debug only)
